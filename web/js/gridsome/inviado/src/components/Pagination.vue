  ...
  <section class="border-b border-gray-300 mx-auto max-w-3xl px-6 md:px-8 py-16">
    <nav role="navigation" aria-label="pagination">
      <ul class="flex items-center justify-between sm:text-lg lg:text-xl">
        <li class="lg:w-1/5">
          <g-link :to="previousPage(info.currentPage)" :class="{'pointer-events-none opacity-0': info.currentPage == 1}" class="text-gray-200 hover:text-black px-4 py-2 transition-color" :rel="info.currentPage == 1 ? 'nofollow' : 'prev'">
            &larr; Previous
          </g-link>
        </li>
        ...
        <li class="lg:w-1/5 text-right">
          <g-link :to="nextPage(info.currentPage,info.totalPages)" :class="{'pointer-events-none opacity-0': info.currentPage == info.totalPages}" class="text-gray-200 hover:text-black px-4 py-2 transition-color" :rel="info.currentPage == info.totalPages ? 'nofollow' : 'next'">
            Next &rarr;
          </g-link>
        </li>
      </ul>
